...
describe('createGang server action', () => {
  ...
  test('creates gang, revalidates, fires analytics, and returns gangId on success', async () => {
    ...
    expect(mockDurationFires).toContainEqual(['createGang', 'user-123'])
  })
...
describe('joinGangByCode server action', () => {
  ...
  test('fires JOIN_REQUESTED analytics event on success', async () => {
    ...
    // R-003: the raw invite code must NOT leak into analytics — funnel
    // attribution lives off gang_id.
    const joinCall = mockTrackEvent.mock.calls.find(
      (call) => call[1] === 'join_requested',
    )
    expect(joinCall?.[2]).not.toHaveProperty('invite_code')
  })
