...
const mockWithTiming = vi.fn(
  async <T,>(actionName: string, userId: string, fn: () => Promise<T>): Promise<T> => {
    try {
      ...
      // R-016: record the duration "fire" so tests can assert the
      // wrapper actually ran around the action body. The real
      // `withTiming` fires SERVER_ACTION_DURATION here unconditionally.
      try {
...
describe('updateDisplayName server action', () => {
  ...
  test('updates name, revalidates, fires analytics, and returns success when user has no gangs', async () => {
    ...
    // R-006: display_name must NOT be sent as an analytics property. The
    // event fires with the distinctId only.
    expect(mockTrackEvent).toHaveBeenCalledWith('user-123', 'display_name_updated')
    const displayCall = mockTrackEvent.mock.calls.find(
      (call) => call[1] === 'display_name_updated',
    )
    expect(displayCall?.[2]).toBeUndefined()
    ...
    expect(mockDurationFires).toContainEqual(['updateDisplayName', 'user-123'])
