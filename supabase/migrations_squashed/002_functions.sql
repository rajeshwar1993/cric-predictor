-- Bragg — 002 Functions, Triggers & Views
...
-- Invite code lookup (bypasses RLS so non-members can find a group to join)
...
-- Deadline calculation helper (used by prediction RLS policies)
...
-- ──────────────────────────────────────────────
-- Group Creation (atomic — group + owner in one transaction)
-- ──────────────────────────────────────────────
...
-- Enforce max members per group
CREATE OR REPLACE FUNCTION enforce_group_capacity()
RETURNS TRIGGER AS $$
DECLARE
  v_max_members CONSTANT INT := 10;
  v_current_count INT;
BEGIN
...
-- ──────────────────────────────────────────────
-- Scenario Seeding & Resolution
-- ──────────────────────────────────────────────

-- Seed system scenarios for a group+match (idempotent)
...
-- Progressive resolution by category (used by edge function during live matches)
...
-- Full match resolution (called after match completes)
