...
export function GangCard({ gang }: GangCardProps) {
  return (
    <Link
      href={`/group/${gang.id}`}
      className={cn(
        'group/card flex min-h-[80px] flex-col justify-center gap-[var(--sp-2)] rounded-[length:var(--radius-ds-lg)] border border-[var(--border-default)] bg-[var(--bg-raised)] p-[var(--sp-4)] transition-colors hover:border-[var(--border-strong)] focus-visible:border-[var(--border-focus)] focus-visible:ring-2 focus-visible:ring-[var(--brand-muted)] focus-visible:outline-none',
      )}
    ...
    >
      <div className="flex items-center justify-between">
        ...
        {gang.role === 'admin' && (
          <span
            className="inline-flex shrink-0 items-center gap-1 rounded-[length:var(--radius-ds-sm)] px-2 py-1 text-xs font-medium uppercase tracking-widest"
            style={{
